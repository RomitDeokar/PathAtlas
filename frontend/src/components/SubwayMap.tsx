...
export function SubwayMap({trace,selected,onSelect,route,removed=[],surviving,knockout,compact}: Props) {
  ...
  const thickness = scaleSqrt().domain([1,300]).range([1,5]);
  ...
  return <div className={`map-frame ${compact?'compact':''}`} ref={frame}>
    ...
    <div className="map-scroll"><svg className="subway" viewBox="0 0 860 420" style={{width:`${zoom*100}%`,minWidth:compact?580:650}} aria-label="Interactive synthetic sensorimotor pathway map">
      <defs><pattern id={`dots-${id}`} width="18" height="18" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r=".65" fill="#d9ded6"/></pattern></defs><rect width="860" height="420" fill={`url(#dots-${id})`}/>
      {stages.map((s,i)=><g key={s}><rect x={i*175+7} y="20" width="145" height="24" rx="4" fill="#f5f6f1"/><text x={80+i*175} y="36" textAnchor="middle" className="stage-label">{s}</text><line x1={80+i*175} x2={80+i*175} y1="57" y2="360" stroke="#e1e5dc" strokeDasharray="3 6"/></g>)}
      {trace.edges.map(e=>{ const key=`${e.source}:${e.target}`; const dead=removed.includes(e.source)||removed.includes(e.target); const alive=remaining?.has(key); const highlight=active?.has(key); const hot=hover===key; const color=dead?'#d18b80':alive?'#4f9974':e.sign==='inhibitory'?'#c99563':e.sign==='unknown'?'#a7afbc':'#70a591'; return <g key={key} onMouseEnter={()=>setHover(key)} onMouseLeave={()=>setHover(null)}>
        ...
        <path d={curve({source:points.get(e.source)!,target:points.get(e.target)!})||''} fill="none" stroke="transparent" strokeWidth="15"><title>{e.source.split('-').at(-1)} → {e.target.split('-').at(-1)}: {e.weight} synthetic count; {e.sign}</title></path>
      ...
      {trace.nodes.map(n=>{ const [x,y]=pos(n); const isSelected=selected===n.id; const dead=removed.includes(n.id); const onPath=route?.nodes.includes(n.id); return <g className="graph-node" key={n.id} transform={`translate(${x},${y})`} tabIndex={0} role="button" aria-label={`${knockout?'Toggle lesion':'Inspect'} ${n.label}, ${n.role}`} aria-pressed={dead||isSelected} onClick={()=>onSelect(n)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onSelect(n);}}}>
        ...
        <rect x="-27" y="25" width="54" height="22" rx="5" fill={isSelected?'#e5ece2':'#f9faf6'}/><text textAnchor="middle" y="41" className="node-label">{n.label}</text><text textAnchor="middle" y="58" className="node-meta">{n.stage===4?n.region.replace(' motor pool','').toLowerCase()+' · fixture':'synthetic cell'}</text>
      </g>;})}
      <text x="430" y="407" textAnchor="middle" className="map-footnote">SCHEMATIC STAGES · SYNTHETIC IDENTIFIERS & WEIGHTS · NOT AN ANATOMICAL RECONSTRUCTION</text>
    </svg></div>
